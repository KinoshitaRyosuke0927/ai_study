...
├── agenda_template.txt ... アジェンダのひな形(自分で編集可能)
...
- GROUPSESSIONのログインID・パスワードは `.env` には設定しません。GROUPSESSIONタブの画面から入力してください(4-2参照)。アプリを終了するとログイン情報は破棄されるため、次回起動時は再度ログインが必要です。
...
チャンネルの表示名を間違えると、該当機能でエラーメッセージが表示されます(2-2参照)。

### 2-3. `agenda_template.txt` の設定

アジェンダタブで生成されるアジェンダのひな形です。`{{YEAR}}`(年)・`{{MONTH}}`(月)・`{{AGENDA_BODY}}`(AIが生成した全体共有事項)の3つのプレースホルダー以外は自由に編集できます。編集後はアプリの再起動は不要で、次回「アジェンダを作成」した際から反映されます。
...
画面上部のタブで「Mattermost」「GROUPSESSION」「アジェンダ」を切り替えます。各タブの右側にある、上下2つのエリアの境界線はドラッグして高さを調整できます。

画面右上の「GROUPSESSIONログイン」ボタンの右に「フィルタ強度」スライダーがあります(0~1の範囲で0.1刻み、初期値0.9)。Mattermost投稿・GROUPSESSION記事を取得すると、AI(ローカルで動作する学習済みモデル)が各投稿・記事に「リマインドが必要そうか」を0~1のスコアで判定し、このスライダーで設定した値以上のスコアのものだけが一覧に表示されます。値を大きくするほど絞り込みが厳しくなり、小さくするほど多くの投稿・記事が表示されます。横の「?」アイコンにカーソルを合わせると説明が表示されます。

### 4-1. Mattermostタブ

1. 左上のプルダウンからチャンネルを選び、取得開始日・取得終了日を指定して「履歴を取得」をクリックします(AIが「フィルタ強度」スライダーの値以上のリマインド必要度スコアを持つ投稿のみに自動で絞り込んで一覧表示します)。
...
1. 画面左上の「GROUPSESSIONログイン」ボタンをクリックするとモーダルが開きます。ID・パスワードを入力して「ログイン」をクリックしてください。ログインに成功するとモーダルが閉じ、ボタンの文字が緑色の「GROUPSESSIONログイン済」に変わります。失敗した場合はモーダル内にエラーメッセージが表示されます。
   - ログイン情報はアプリ(サーバ)のメモリ上にのみ保持され、ファイルには保存されません。アプリを終了すると破棄されるため、次回起動時は再度ログインが必要です。
   - 未ログインの間は、GROUPSESSIONタブの「新着記事を取得」・アジェンダタブの「履歴・新着記事を取得」ボタンは非活性になっています。
2. 取得開始日・取得終了日を指定して「新着記事を取得」をクリックします(AIが「フィルタ強度」スライダーの値以上のリマインド必要度スコアを持つ記事のみに自動で絞り込んで一覧表示します)。
3. 左下の一覧から記事をクリックすると、右上に本文(添付ファイル一覧含む)が表示されます。
4. 「リマインドを作成」をクリックすると、AIがリマインド文章を生成します(本文には元記事のURLが自動で付与されます)。
5. 右下の「投稿先」プルダウンで、`DM` または `settings.ini` の `groupsession.remind_channel` に設定したチャンネルへの投稿を選択します。
6. 「投稿」をクリックすると送信されます。
...
2. 「取得した投稿一覧」の右にある「日時順」「投稿者順」「取得元順」ボタンで一覧を並び替えられます(取得元順はGROUPSESSIONが上、Mattermostが下になります)。
3. 左下の一覧から項目をクリックすると詳細が表示され、チェックボックスでアジェンダに含める項目を選択します。「アジェンダを作成」ボタンは、1件以上チェックすると活性化します。
4. 「アジェンダを作成」をクリックすると、チェックした項目をもとにAIが部署定例会の「全体共有事項」を生成し、`agenda_template.txt` のひな形に埋め込んで右下のテキストエリアに表示します。
5. 内容を確認・編集し、「公開先: 年」「公開先: 月」を指定します。「公開範囲」のスイッチで「公開」(誰でも閲覧可能)/「自分のみ公開」(自分だけが閲覧可能)を切り替えられます(初期状態は「公開」)。「wikiへ公開」をクリックすると、`settings.ini` の `growi.root_path` 配下に「年/月」のページとして公開されます(既に同じ年月のページがある場合は上書き更新されます)。
   - 既存ページの公開範囲を「自分のみ公開」に切り替えて公開した場合、ページが新規に作り直されるため**URLが変わります**。以前のURLを共有していた場合は無効になります(「wikiへ公開」ボタン横の「?」アイコンにカーソルを合わせると注意書きが表示されます)。
...
| 「GROUPSESSIONにログインしてください」と表示される | GROUPSESSIONタブでログインしていません。4-2の手順1でログインしてください。 |
| GROUPSESSIONログイン時に「IDまたはパスワードが正しくありません」と表示される | 入力したID・パスワードが誤っています。GROUPSESSIONに実際にログインできる情報を再入力してください。 |
| 「GROUPSESSIONへのアクセスに失敗しました」と表示される | GROUPSESSION側の一時的な不調、通信エラーなどの可能性があります。 |
| 「GROWIへの公開に失敗しました」と表示される | `GROWI_API_TOKEN` が無効、または `growi.root_path` の指定が誤っている可能性があります。 |
| 「モデルによる投稿の絞り込みに失敗しました」と表示される | アプリ内部のAIモデルの読み込みに失敗している可能性があります。`_internal` フォルダが欠損・破損していないか確認し、改善しない場合は配布元に問い合わせてください。 |
| 表示されるべき投稿・記事が一覧に出てこない、または関係のない投稿・記事まで表示される | 「フィルタ強度」スライダーの値を調整してください。値を下げると表示件数が増え(見逃しは減るが不要な投稿も増える)、上げると表示件数が減ります(絞り込みは厳しくなるが見逃しも増える可能性があります)。 |
