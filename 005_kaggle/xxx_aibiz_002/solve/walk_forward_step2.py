"""
Step2: ラグ特徴量（3〜6日前）追加
ベース: walk_forward.py (RMSE 3.1831)
追加特徴量: price_lag_144, price_lag_192, price_lag_240, price_lag_288
"""
import pandas as pd
import numpy as np
...
train_sorted["price_lag_144"]   = train_sorted["tokyo_price"].shift(144)   # 3日前（Step2追加）
train_sorted["price_lag_192"]   = train_sorted["tokyo_price"].shift(192)   # 4日前（Step2追加）
train_sorted["price_lag_240"]   = train_sorted["tokyo_price"].shift(240)   # 5日前（Step2追加）
train_sorted["price_lag_288"]   = train_sorted["tokyo_price"].shift(288)   # 6日前（Step2追加）
...
lag_cols = [
    ...
    "price_lag_144", "price_lag_192", "price_lag_240", "price_lag_288",  # Step2追加
...
submission.to_csv("submission_step2.csv", index=False)
print(f"\n予測完了: submission_step2.csv ({len(submission)}件)")
