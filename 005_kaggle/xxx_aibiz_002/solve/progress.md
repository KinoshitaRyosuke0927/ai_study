...
| ピーク/オフピーク分離モデル | walk_forward_v3.py | 8.6963 | +0.0223 |
...
### walk_forward_v3.py（RMSE: 8.6963）
- **アプローチ**: ピーク/オフピーク分離モデル ＋ ウォークフォワード予測 ＋ ノイズ注入
- **分離定義**:
  - ピーク   : `is_holiday=0` かつ `dayofweek=0-4` かつ `frame=17〜40`（平日8:00〜20:00）
  - オフピーク: それ以外（休日・祝日の全コマ、平日の深夜〜早朝・夜間コマ）
- **学習数**:
  - ピーク: 学習データの約36%（平日コマ17-40）
  - オフピーク: 残り約64%
- **モデル構成**: 各セグメントで `NOISE_STD=25.0 × 5モデル` のアンサンブル（walk_forward_v2 と同設定）
- **walk_forward予測**: 各コマをピーク/オフピーク判定し、対応モデルで予測。price_buffer は全コマ共有
- **特徴量**: walk_forward_v2 と同一（base_features ＋ lag_48/96/336/17520）
...
| ピーク/オフピーク分離モデル（walk_forward_v3） | 悪化（8.6963） | LightGBMはframe×dayofweekの交互作用を自動学習するため分割の恩恵より学習データ減少（ピーク側36%）のデメリットが上回った |
