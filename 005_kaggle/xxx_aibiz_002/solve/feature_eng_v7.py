"""
JEPX 東京エリア電力スポット価格予測 - 特徴量エンジニアリング v7（LightGBM）
v4 からの変更点:
  1. price_lag_35040 を除外（重要度0のため）
  2. weather 列を補間して有効化
       欠損パターン: 数時間ごとの観測のため、観測値の間がNaN
       補間方法: 前方補間（ffill）→ 残余を後方補間（bfill）
       エンコード: ラベルエンコーディングで整数変換
"""
import pandas as pd
import numpy as np
...
# 前方補間 → 後方補間で欠損を埋める（天気は数時間ごとに観測されるため）
combined_weather["weather"] = combined_weather["weather"].ffill().bfill()

# ラベルエンコーディング（学習・テスト間で同じマッピングを適用）
weather_categories = sorted(combined_weather["weather"].dropna().unique())
weather_map = {w: i for i, w in enumerate(weather_categories)}
...
print("weather エンコードマッピング:")
for w, i in sorted(weather_map.items(), key=lambda x: x[1]):
    print(f"  {i}: {w}")
...
submission.to_csv("submission_feature_eng_v7.csv", index=False)
print(f"\n予測完了: submission_feature_eng_v7.csv ({len(submission)}件)")
