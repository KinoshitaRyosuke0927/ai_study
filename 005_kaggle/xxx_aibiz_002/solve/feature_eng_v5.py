"""
JEPX 東京エリア電力スポット価格予測 - 特徴量エンジニアリング v5（LightGBM）
v4 に前年同期比の差分特徴量を追加
追加特徴量:
  - price_diff_yoy  : 当期価格 - 1年前価格（前年同期比の変化量）
  - price_diff_2yoy : 1年前価格 - 2年前価格（前年比トレンドの方向性）
"""
import pandas as pd
import numpy as np
...
# 前年同期比の差分: 価格トレンドの変化量を表現
# price_diff_yoy  = 当期価格 - 1年前価格
# price_diff_2yoy = 1年前価格 - 2年前価格（前年比のトレンド方向）
combined["price_diff_yoy"]  = combined["tokyo_price"]           - combined["price_lag_17520"]
combined["price_diff_2yoy"] = combined["price_lag_17520"]       - combined["price_lag_35040"]

lag_cols = ["id", "price_lag_17520", "price_lag_35040",
            "price_diff_yoy", "price_diff_2yoy"]
...
submission.to_csv("submission_feature_eng_v5.csv", index=False)
print(f"\n予測完了: submission_feature_eng_v5.csv ({len(submission)}件)")
