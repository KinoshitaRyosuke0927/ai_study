"""
JEPX 東京エリア電力スポット価格予測 - 特徴量エンジニアリング v2（LightGBM）
feature_selection.py による検証で改善が確認された特徴量のみ採用
追加特徴量:
  - supply_demand_ratio : sell_amount / buy_amount（需給比率。1超で供給過多→価格低下）
  - surplus             : sell_amount - buy_amount（需給超過量）
除外した特徴量（CV RMSEが悪化したため）:
  - contract_rate, month, season
"""
import pandas as pd
import numpy as np
...
def add_features(df):
    df = df.copy()
    # 需給比率: 1超で供給過多→価格低下、1未満で供給不足→価格上昇
    df["supply_demand_ratio"] = df["sell_amount"] / df["buy_amount"]
    # 需給超過量: 正値が大きいほど供給が需要を上回り価格が下がる
