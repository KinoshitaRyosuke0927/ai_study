"""
F1 ピットストップ予測 - 外部データ（元F1データセット）活用版
既存コードは一切変更しない。

外部データの概要:
  - aadigupta1601/f1-strategy-dataset-pit-stop-prediction (Kaggle)
  - 101,371 行 / コンペ訓練データと同じカラム + Normalized_TyreLife
  - コンペ訓練データ (439,140行) と統合して約 540K 行で学習

統合方針:
  - Normalized_TyreLife はテストデータに存在しないため使用しない
  - キーが重複する 15,830 行はそのまま両方保持（合成 vs 実データで値が異なる）
  - Compound の欠損 66 行は前後のラップから推定して補完
  - モデルはアンサンブル版と同じ LightGBM (Optuna 最適パラメータ) + CatBoost
"""
import pandas as pd
import numpy as np
...
# 元 F1 データセットをダウンロード（キャッシュがあれば即時完了）
...
# ── 訓練データの統合 ─────────────────────────────────────
train = pd.concat([train_comp, orig], ignore_index=True)
print(f"\n統合後訓練データ: {train.shape}")
print(f"PitNextLap 分布:\n{train[TARGET].value_counts(normalize=True).round(4)}")
...
# train + test の全カテゴリを fit することで未知カテゴリを防ぐ
...
for w in np.linspace(0, 1, 101):
    auc = roc_auc_score(y, w * oof_lgbm + (1 - w) * oof_cb)
    if auc > best_auc:
        best_auc = auc
        best_w   = w
...
print(f"  改善幅 (外部データ追加効果) : {best_auc - 0.94992:+.5f}")
...
out_path   = OUTPUT_DIR / "submission_external_data.csv"
