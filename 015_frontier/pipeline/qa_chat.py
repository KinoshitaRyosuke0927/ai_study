...
- プロジェクト概要・環境構築手順・使い方など、特定の機能に紐づかない質問(例:
  「READMEに何が書いてある?」「セットアップ方法は?」)には、search_design_code では
  見つからないことが多いです。その場合は list_repo_docs でドキュメント一覧を取得し、
  該当しそうなファイルを read_repo_file で読んでください。
